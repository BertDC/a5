...
bool Drow::consumePotion(std::string dir) {
	...
	if (type == 0) {
		loseHp(15);
		floor->actionQueue += " You feel sick! Lost 15 health.";
	}
	else if (type == 1) {
		hp = fmin(maxHp, hp + 15);
		floor->actionQueue += " You feel rejuvenated! Gain +15 health.";
	}
	else if (type == 2) {
		atkMod -= 7.5;
		floor->actionQueue += " You feel tired... Lose 7.5 attack.";
	}
	else if (type == 3) {
		atkMod += 7.5;
		floor->actionQueue += " You feel stronger! Gain 7.5 attack!";
	}
	else if (type == 4) {
		defMod -= 7.5;
		floor->actionQueue += " You feel weak. Lose 7.5 defense.";
	}
	else if (type == 5) {
		defMod += 7.5;
		floor->actionQueue += " You feel your skin harden. Odd. Gain 7.5 defense.";
	}
	else if (type == 6) {
